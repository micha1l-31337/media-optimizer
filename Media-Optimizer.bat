...
set "VID_FPS=30"            
:: Целевая частота кадров (кадры в секунду).
...
set "VID_MAXRATE=20M"       
:: Максимальный битрейт видео (ограничение пиковой нагрузки).
:: Примеры: 50M=4K высокое кач-во, 20M=FullHD высокое, 10M=FullHD стандарт.

set "VID_BUFSIZE=40M"       
:: Размер буфера для контроля битрейта (рекомендуется 2x от MAXRATE).
...
:: Аудиокодек ffmpeg (рекомендуется aac или устаревший mp3 для максимальной совместимости).

set "VID_AUDIO_BITRATE=192k"
:: Битрейт аудио (качество звука).
:: Примеры: 320k=высокое кач-во, 192k=стандарт (CD-качество).

set "SUFFIX_RESIZED=_resized"
:: Суффикс для переименования обработанных файлов, можно оставить пустым.
:: Пример: "photo.jpg" → "photo_resized.jpg"
 
set "AUTO_BACKUP_ROOT=" 
:: Корневая папка для сохранения результатов, например F:\PHOTO\
...
set "IN_DIR=%CD%"           
:: Папка-источник по умолчанию (текущая папка скрипта).
